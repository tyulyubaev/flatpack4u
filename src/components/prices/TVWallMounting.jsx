...
export default class TVWallMounting extends Component {
  ...
  verification = () => {
    if (this.state.tvsize === "") {
      this.setState({ tvsizeVal: false });
    } else {
      this.setState({ tvsizeVal: true });
    }
    if (this.state.wall === "") {
    ...
    if (this.state.tvsize !== "" && this.state.wall !== "") {
      this.calculations();
    }
  };
  calculations = () => {
...
class TVMounting extends Component {
  render() {
    ...
    const inputSize =
      this.props.data.tvsizeVal === false
